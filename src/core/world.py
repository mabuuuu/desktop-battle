...
class World:
    ...
    def _create_unit(self, faction: Faction, phys_x: float, phys_y: float) -> Unit:
        ...
        self.physics.add_body(unit.body, unit.shape)  # type: ignore[arg-type]
        return unit
    ...
    def _create_resource_node(
        ...
        self.physics.add_body(node.body, node.shape)  # type: ignore[arg-type]
        return node

    def create_building(
        self, faction: Faction, building_type: str, level: int, phys_x: float, phys_y: float
    ) -> Building | None:
        """创建建筑 (工具台/兵营)."""
        ...
        self.physics.add_body(building.body, building.shape)  # type: ignore[arg-type]
        return building
    ...
    def update(self, dt: float) -> None:
        ...
        for unit in self.units:
            if unit.alive:
                ...
                # 简单的 idle 行为: 如果在地面上且空闲，微动
                if unit.state == UnitState.IDLE and unit.on_ground:
                    pass  # 站立不动
    ...
    def _cleanup_dead_units(self) -> None:
        ...
        for unit in self.units:
            if not unit.alive and unit.body is not None:
                # 从物理空间移除
                try:
                    self.physics.remove_body(unit.body, unit.shape)  # type: ignore[arg-type]
                except Exception:
                    pass
                unit.body = None
                unit.shape = None
    ...
    def _render_hud(self, buffer: np.ndarray) -> None:
        ...
        y_offset = 30
        for faction in self.factions:
            info = f"{faction.name}: U={faction.alive_count} W={faction.wood} O={faction.ore}"
